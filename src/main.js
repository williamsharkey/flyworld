...
<aside class="sidebar"><section class="panel observer-panel"><div class="panel-heading"><span>THE OBSERVER</span><span class="subtle">01 / DROSOPHILA</span></div><div class="observer-title"><div class="fly-small">${icon("bug")}</div><div><strong>A curious explorer</strong><small>Eyes forward. Mind wandering.</small></div><span class="proxy" title="Compact spiking proxy, not the measured FlyEM connectome">PROXY</span></div><div class="neural-view"><canvas id="neural" width="464" height="145" aria-label="Live spiking neural activity"></canvas><span class="neural-label">892 VISUAL UNITS</span></div><div class="metrics"><div><div class="metric-title">${icon("zap")} Neural activity</div><div class="metric-value"><span id="spike-rate">0</span><small>spikes/s</small></div><canvas class="sparkline" id="spike-chart" width="210" height="56"></canvas></div><div><div class="metric-title">${icon("sparkles")} Reward proxy</div><div class="metric-value"><span id="dopamine">0.50</span><small>DA</small></div><canvas class="sparkline" id="reward-chart" width="210" height="56"></canvas></div></div><div class="observer-bottom"><span class="status-dot"></span><span id="brain-status">Learning from the view</span></div></section>
<section class="panel eye-panel"><div class="panel-heading"><span>THROUGH ITS EYES</span>${icon("scan-eye")}</div><div class="eye-head"><span>892 columns · 30 × 30 light samples</span><span>LIVE</span></div><div class="eye-wrap"><canvas id="eye" width="30" height="30" aria-label="Actual low-resolution view used by the fly's visual model"></canvas><div class="eye-caption"><span>LEFT FIELD</span><span>RIGHT FIELD</span></div></div><div class="steering"><span>← L</span><div class="steering-track"><span id="steer-dot"></span></div><span>R →</span><span id="steer-label">Seeking balance</span></div><div class="novelty-readout"><span>Visual novelty <b id="novelty">100%</b></span><span><b id="swap-count">0</b> form swaps</span></div></section></aside>
...
<div class="bottom-area"><section class="orbit-panel map-panel"><div class="map-heading"><span>FLIGHT MEMORY</span><span id="lap-number">Loop 001</span></div><canvas id="orbit" width="480" height="160" aria-label="Explored paths on an unwrapped torus; repeated visits become brighter"></canvas><div class="map-meta"><strong id="coverage">0.00% touched</strong><span id="visited-cells">1 cell</span></div><div class="map-key"><span>FAINT → FAMILIAR</span><span id="loop-time">05:00 / lap</span></div><span id="loop-progress" hidden></span></section><div class="center-bottom"><div class="event">${icon("sprout")}<span id="event-text">A small world. An open-ended experiment.</span></div><nav class="controls" aria-label="Simulation controls"><button id="pause" class="control-button primary" title="Pause (Space)" aria-label="Pause simulation">${icon("pause")}</button><button id="speed" class="control-button" title="Change simulation speed"><span id="speed-label">1×</span>${icon("chevrons-right")}</button><span class="control-sep"></span><button id="camera" class="control-button active" title="Change camera (C)">${icon("video")}<span class="camera-label" id="camera-label">Follow</span></button><button id="mutate" class="control-button mutate" title="Mutate the landscape ahead (M)">${icon("shuffle")}<span>Mutate</span></button><span class="control-sep"></span><button id="settings" class="control-button icon-only" title="Evolution settings" aria-label="Evolution settings" aria-expanded="false">${icon("sliders-horizontal")}</button><button id="sound" class="control-button icon-only" title="Enable ambient sound" aria-label="Enable ambient sound" aria-pressed="false">${icon("volume-x")}</button><button id="immersive" class="control-button icon-only" title="Screensaver mode (F)" aria-label="Enter screensaver mode">${icon("maximize")}</button></nav><section class="ecosystem"><div class="eco-top"><span>12 WORLD FORMS · ENDLESS COMBINATIONS</span><b><span class="status-dot"></span> &nbsp; 8 interdependent species</b></div><div class="species-list">${SPECIES.map((s, i) => `<button class="species-button" data-species="${i}" style="--species:${s.color}" title="Explore ${s.name}"><span class="species-name"><span class="species-dot"></span>${s.short}</span><div class="species-bar"><div class="species-fill" id="species-${i}"></div></div></button>`).join("")}</div></section></div><section class="world-stats"><div class="world-stats-heading"><span>NATURAL SELECTION, REIMAGINED</span>${icon("dna")}</div><div class="stat-row"><span>Generation</span><strong id="generation">001</strong></div><div class="stat-row"><span>Ecosystem fitness</span><strong id="fitness">—</strong></div><div class="stat-row"><span>Living voxels</span><strong id="voxels">—</strong></div><div class="tiny-footer"><span>BUILT WITH THREE.JS</span><a href="https://github.com/fruitflydev/flycoinrh" target="_blank" rel="noopener noreferrer">INSPIRED BY FLYBRAIN ↗</a></div></section></div><div class="corner-note">A QUIET EXPERIMENT IN ARTIFICIAL LIFE</div>
...
$("speed").onclick = () => {
  ...
  $("loop-time").innerHTML =
    `${formatTime(300 / speed)} <span style="font-size:10px;color:#aab89c">/ lap</span>`;
  toast(`Moving at ${speed}× · ${formatTime(300 / speed)} per lap`);
};
...
function mutate() {
  const changed = ecosystem.evolve(world.u, world.v, brain.dopamine, true);
...
$("sound").onclick = async () => {
  ...
  try {
    ...
    $("sound").setAttribute(
      "aria-label",
      on ? "Mute synth soundtrack" : "Enable synth soundtrack",
    );
    $("sound").title = on
...
function updateUI() {
  ...
  $("lap-number").textContent =
    `Loop ${String(Math.floor(time / 300) + 1).padStart(3, "0")}`;
...
function animate(now) {
  ...
  world.update(dt, autosteer ? brain.steer : 0, realDt);
  if (dt) {
    senseTime += dt;
    evoTime += dt;
  }
  ...
  while (evoTime >= 0.85) {
    ecosystem.observe(world.u, world.v, brain.novelty, 0.85);
    world.markDirty(ecosystem.evolve(world.u, world.v, brain.dopamine));
    evoTime -= 0.85;
  }
