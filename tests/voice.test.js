...
import { RobotVoice, VOCABULARY, VERSE_BARS, CHANT_WORDS, VOICE_VOLUME, vocalBar } from "../src/robot-voice.js";
import { CombatAudio, COMBAT_BPM } from "../src/combat-audio.js";
import { SCORE } from "../src/score.js";

test("every verse syllable has phonemes and fits the eighth-note grid", () => {
  assert.equal(VERSE_BARS.length, 8);
  for (const bar of VERSE_BARS) {
    ...
    bar.words.forEach((w, i) => {
      ...
      assert.equal(w.beat, i * 0.5);
      assert.ok(Number.isFinite(w.note));
    });
  }
  assert.equal(vocalBar(0).words, CHANT_WORDS);
  assert.equal(vocalBar(2), VERSE_BARS[0]);
  assert.equal(vocalBar(9), VERSE_BARS[7]);
  assert.equal(vocalBar(12).words, CHANT_WORDS);
});
...
test("scheduler cycles through every verse without late bunching or drifting off beat two", () => {
  ...
  Object.assign(c, { startedAt: 0, until: 100, chantOrigin: null, chantLoop: 0, chantIndex: 0,
    voice: { word: (...args) => calls.push(args) } });
  for (let now = 12; now < 36; now += 0.05) c.scheduleChant(now);
  assert.ok(calls.some(c => c[0] === "big"));
  ...
  calls.length = 0;
  c.scheduleChant(40);
  assert.ok(calls.every(c => c[1] >= 40));
});
