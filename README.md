...
**Firing soundtrack:** only actual Space-triggered shots activate the original 150 BPM electro / Miami-bass groove. The quiet score plays at 120% of its former gain and ducks to 12% under distorted 808 kicks, sub-bass, claps, and hats. Collision and automatic-clearance explosions never activate it. After eight seconds in a continuous firing session, a monophonic 303-style acid line joins the groove: sawtooth oscillator, resonant cascaded low-pass filters, distortion, accents, connected gates, and exponential portamento. It uses an original 32-step pattern with tied notes and slides, probabilistic ornamentation, and modal pitch mapping. Acid output is 70% of its former level. After twelve seconds, locally generated Kokoro neural spoken/chant vocals enter on beat two. A clear natural-pitch lead carries the consonants; quiet lower- and upper-octave layers add color. Vocal gain is multiplied by 0.55 relative to the previous setting (0.7 → 0.385). The browser downloads a 1.8 MB audio atlas, not a neural model. Whole phrases start on an eight-beat grid; pitch-preserving offline tempo fitting retains natural speech cadence and leaves space at the end of each line. Acid and melodic details ease back while the lead speaks.

Each song family has three eight-line verses, four hook lines and two bridge lines: 60 lines total. An arrangement plays a sixteen-bar verse, four-bar instrumental answer, eight-bar hook, four-bar bridge and four-bar turnaround. Drum density and instrumental energy approach each section gradually. Verse chapters advance across sessions; sustained sessions switch song families with the larger opus at completed arrangement boundaries. A three-second firing gap ends the session, stops vocals, and restores the ambient score; a new session restarts the twelve-second timer and starts verse-first. Late frames skip expired phrases rather than bunching words together. If audio is still loading, the next verse waits for an aligned entrance. A failed vocal download does not interrupt instruments and retries on the next firing session.

The lyrics and compositions are original, with the user's requested hook wording. No artist recording or cloned artist voice is used. Vocal provenance is documented in [public/audio/README.md](public/audio/README.md). To regenerate the bundled audio, create a Python 3.12 virtual environment, install `kokoro-onnx==0.6.1` and `soundfile`, and install `ffmpeg`. Download `kokoro-v1.0.onnx` and `voices-v1.0.bin` from the [kokoro-onnx v1.0 model release](https://github.com/thewh1teagle/kokoro-onnx/releases/tag/model-files-v1.0), then run:

```sh
python scripts/render-vocals.py --model /path/kokoro-v1.0.onnx --voices /path/voices-v1.0.bin
```

Generation is an offline production step; normal builds and GitHub Pages do not require Python or model files.
...
- `src/combat-audio.js` / `src/acid.js` / `src/robot-voice.js`: firing-only bass groove, delayed acid layer, ambient ducking, and beat-scheduled neural voice clips.
- `src/song-form.js` / `src/lyrics.json`: song sections, verse progression, and original lyrics.
- `scripts/render-vocals.py` / `public/audio/`: reproducible offline neural-vocal production and the shipped audio atlas.
